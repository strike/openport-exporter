...
| `scans_failed_total`            | Total number of scans that encountered an error for a given target, port range, and protocol                        | `scans_failed_total{target="192.168.1.0/24", port_range="22-80", protocol="tcp"} 1`                         | Counter  |
...
- **`nmap_host_up_count`**  
  - **Description**: Shows how many hosts are reported as up for a given target during the last scan. Useful for understanding partial network reachability.  
  - **Example**: `nmap_host_up_count{target="192.168.1.0/24"} 10`  
  - **Type**: Gauge  

- **`nmap_host_down_count`**  
  - **Description**: Shows how many hosts are reported as down (unreachable) for a given target during the last scan.  
  - **Example**: `nmap_host_down_count{target="192.168.1.0/24"} 5`  
  - **Type**: Gauge  

- **`scans_successful_total`**  
  - **Description**: Counts the total number of scans that finished without errors for a particular target, port range, and protocol.  
  - **Example**: `scans_successful_total{target="192.168.1.0/24", port_range="22-80", protocol="tcp"} 3`  
  - **Type**: Counter  

- **`scans_failed_total`**  
  - **Description**: Counts the total number of scans that encountered an error for a particular target, port range, and protocol.  
  - **Example**: `scans_failed_total{target="192.168.1.0/24", port_range="22-80", protocol="tcp"} 1`  
  - **Type**: Counter  

- **`last_scan_timestamp_seconds`**  
  - **Description**: Shows the Unix timestamp of the most recent scan for a target, port range, and protocol. Helps verify whether scans are running on schedule.  
  - **Example**: `last_scan_timestamp_seconds{target="192.168.1.0/24", port_range="22-80", protocol="tcp"} 1695051127`  
  - **Type**: Gauge  

---

### Explanation of the Metric `open_ports_total`
